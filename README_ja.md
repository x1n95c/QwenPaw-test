...
- [2026-05-14] **v1.1.7** をリリースしました！詳細は [v1.1.7 リリースノート](https://qwenpaw.agentscope.io/release-notes) をご覧ください。
  ...
  - **[v1.1.7] 新機能**：受信トレイ（承認とプッシュメッセージ）；Cron ジョブ強化（ワンタイム実行、カレンダービュー、組み込みテンプレート、実行履歴とトレース）；Browser Use バッチアクションとファイルダウンロード；リモート MCP OAuth 2.1 認証；コンソールプラグイン管理；Qwen-Image・Wan 2.7 プラグイン；外部エージェント非同期実行。
  - **[v1.1.7] 変更**：モデルセレクターを検索可能なフラットリストに刷新；フローティングチャットボタン。
  - **[v1.1.7] パフォーマンス**：ファイル読み取り時のメモリ使用量を最適化；Keyring タイムアウト保護。
  - **[v1.1.7] 修正**：セッション履歴ルーティング；Volcengine モデル ID；Feishu WebSocket キープアライブ。
  - **[v1.1.7] 新規コントリビューター**：@weixizi、@StarTrekking、@aqilaziz、@suntp の皆さん、ありがとうございます。
